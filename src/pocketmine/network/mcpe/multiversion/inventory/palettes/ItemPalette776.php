...
final class ItemPalette776 extends Palette{
	/** @var int[] */
	...
	public static function init() : void{
		/** @var int[] $itemPalette */
		$itemMap = json_decode(file_get_contents(RESOURCE_PATH . "/vanilla/items/776/r16_to_current_item_map.json"), true);

		/** @var int[] $itemPalette */
		$stringToIntMap = json_decode(file_get_contents(RESOURCE_PATH . "/vanilla/item_id_map712.json"), true);
		...
		$itemList = json_decode(file_get_contents(RESOURCE_PATH . "/vanilla/items/776/required_item_list.json"), true);
		...
		foreach($itemList as $stringId => $entry){
			$entries[] = new ItemTypeEntry(
				...
				$entry["version"],
				isset($entry["component_nbt"]) ? $nbtSerializer->read(base64_decode($entry["component_nbt"])) : $emptyNBT
			);
		}
