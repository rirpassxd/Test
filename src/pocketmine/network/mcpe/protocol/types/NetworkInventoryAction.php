...
class NetworkInventoryAction{
	...
	public function read(NetworkBinaryStream $packet, bool $hasItemStackIds){
        ...
        if($packet->getProtocol() >= ProtocolInfo::PROTOCOL_392 && $packet->getProtocol() < ProtocolInfo::PROTOCOL_429){
	    	if($hasItemStackIds){
		...
		switch($this->sourceType){
			...
			case self::SOURCE_TODO:
				...
				switch($this->windowId){
					...
					case self::SOURCE_TYPE_ANVIL_INPUT: // на новых версиях это удаление предмета из маяка...
						$window = $player->getWindow(Player::BEACON_WINDOW_ID);
						if(!($window instanceof BeaconInventory)){
							return null;
						}
