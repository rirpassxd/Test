...
class Fireworks extends Item{
	...
	public function onActivate(Player $player, Block $blockReplace, Block $blockClicked, int $face, Vector3 $clickVector) : bool{
		...
		$entity = Entity::createEntity("FireworksRocket", $player->getLevelNonNull(), $nbt, $this);
	...
	public function onClickAir(Player $player, Vector3 $directionVector) : bool{
		if($player->isGliding()){
			...
			if($entity instanceof Entity){
				...
				$player->getLevelNonNull()->addSound(new BlazeShootSound($player));
			}
		}
    ...
    public function getItemProtocol(int $protocol) : ?Item{
        if($protocol < ProtocolInfo::PROTOCOL_130){
            return ItemFactory::get(ItemIds::NETHER_STAR, $this->getDamage());
        }
