...
class ItemFactory{
	...
	public static function init(){
        ...
        self::registerItem(new NetheriteSword(), true);
        self::registerItem(new NetheritePickaxe(), true);
        self::registerItem(new NetheriteAxe(), true);
        self::registerItem(new NetheriteShovel(), true);
        self::registerItem(new NetheriteIngot(), true);
        self::registerItem(new NetheriteHoe(), true);
	...
	public static function get(int $id, int $meta = 0, int $count = 1, $tags = null) : Item{
		...
		if(isset(self::$list[self::getListOffset($id)])){
			...
			if($item instanceof Durable){
				if($meta < 0){
					$item->setDamage(0);
				}elseif($meta > $item->getMaxDurability()){
		...
		}elseif($id < 256){ //intentionally excludes negatives because extended blocks aren't supported yet
			...
			$item = new ItemBlock($id, $meta === -1 ? -1 : $meta & 0xf);
		}else{
	...
	public static function isRegistered(int $id) : bool{
		...
		return isset(self::$list[self::getListOffset($id)]);
	}
