...
class Server{
	...
	/**
	 * @param ThreadSafeClassLoader     $autoloader
	...
	public function __construct(ThreadSafeClassLoader $autoloader, ThreadSafeLogger $logger, string $dataPath, string $pluginPath){
		...
		try{
			...
			if(file_exists(\pocketmine\RESOURCE_PATH . "misoft_$lang.yml")){
				$content = file_get_contents($file = \pocketmine\RESOURCE_PATH . "misoft_$lang.yml");
			}else{
				$content = file_get_contents($file = \pocketmine\RESOURCE_PATH . "misoft_rus.yml");
			}

			if(!file_exists($this->dataPath . "misoft.yml")){
				@file_put_contents($this->dataPath . "misoft.yml", $content);
			}
			$this->advancedConfig = new Config($this->dataPath . "misoft.yml", Config::YAML, []);
			...
			Enchantment::init();
			ItemFactory::init();
			CreativePacketCache::getInstance(); // init creative items
			Biome::init();
			...
			$this->bedrockResourcePacksProtocol = (int) $this->getProperty("bedrock-resource-packs-protocol", ProtocolInfo::PROTOCOL_370);
	...
	public function loadAdvancedConfig() : void{
		...
		$this->loadIncompatibleAPI = $this->getAdvancedProperty("developer.load-incompatible-api", false);
